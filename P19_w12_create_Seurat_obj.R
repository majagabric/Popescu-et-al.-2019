...
data10x <- Read10X('/Users/majagabric/Documents/PhD/Data analysis/Popescu et al., 2019/Popescu et al., 2019/aggrData/Pop12_aggr/count/filtered_feature_bc_matrix')
seurat_obj_w12 <- CreateSeuratObject(data10x, 
                                    min.cells = 3, min.features = 200, 
                                    project = "Popescu2019_w12")

##### Add metadata #####
aggr_metadata <- read.csv('/Users/majagabric/Documents/PhD/Data analysis/Popescu et al., 2019/Popescu et al., 2019/aggrData/Pop12_aggr/aggregation.csv')

barcode_suffix <- sub(".*-(\\d+)$", "\\1", colnames(seurat_obj_w12))
...
names(sample_id) <- colnames(seurat_obj_w12)

# add sample_id to the metadata
seurat_obj_w12 <- AddMetaData(seurat_obj_w12, metadata = sample_id, col.name = "sample_id")

# Get the cell tags
barcodes <- colnames(seurat_obj_w12)
...
seurat_obj_w12$Sample <- sapply(strsplit(barcodes, "-"), function(x){return(x[2])})
...
seurat_obj_w12$Sample <- as.numeric(seurat_obj_w12$Sample)
sex_metadata <- "F"
seurat_obj_w12$Sex <- sex_metadata[seurat_obj_w12$Sample]

# Add Week_gestation and Tissue
seurat_obj_w12$Week_gestation <- 12
seurat_obj_w12$Tissue <- "FL"

##### Save Seurat object #####
saveRDS(seurat_obj_w12, file="P19_w12_seurat_obj_preQC.rds")

##### Load Seurat object #####
seurat_obj_w12 <- readRDS('/Users/majagabric/Documents/PhD/Data analysis/Popescu et al., 2019/Popescu et al., 2019/P19_w12_seurat_obj_preQC.rds')
